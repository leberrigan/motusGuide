...
This guide was designed to help both researchers and public users access data available on Motus.org. It will be split into four distinct sections:
...
Tags are the radio transmitters that are affixed to animals for tracking. These tags emit a coded 'burst' of radio pulses that encode a unique ID for that tag. There are a limited number of unique IDs that can be produced, however, making it necessary to find other way to expand the number of uniquely identifyable tags. This is done by also calculating the length of time between each coded burst, called the **burst interval**. By combining the burst interval with the encoded ID of the tag, the number of uniquely identifyable tags is theoretically unlimited. But in practice, the burst interval is limited by the precision of measurement and accuracy of the tag. In addition, it is impractical to have tags with very long burst intervals since this will reduce the number of detections and makes manual tracking nearly impossible. For this reason, there are many cases of tags with identical properties, but we take care to ensure these tags are not deployed in the same region or at the same time.
...
<div class='note'>
<span>Note:</span> 
This is especially important for users who wish to conceal the location of tags and tag detections for rare species or when landowners do not want to have their location publically available.</div>
...
Tag finder only searches for tags that are included in its master tag list which is just a huge list of registered tags. Importantly, this table contains deployment dates for when tag finder can should be finding tags. If any tag detections occur outside of these dates they will not be added to the database. If the user has not set a deployment start date, the date of registration is used. If the user has not entered an end date, the deployment start date plus the estimated tag lifespan is used.     
